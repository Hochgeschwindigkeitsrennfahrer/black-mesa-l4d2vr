...
if (-not (Test-Path $CfgDst)) {
  ...
  if ($cfgText -notmatch '(?m)^CapEyesToWindow=') { $cfgText += "`r`nCapEyesToWindow=true`r`n" }
...
Write-Host "Installed $BmvrCfgDst (CSM off, Xen LOD, vis restored, no flashlight cvars)"
