    ...
    bool)
{
    ...
    // The 32-bit game process does not own the OpenXR session. L4D2VR's
    // x64 helper does. A missing Win32 openxr_loader.dll must not block
    // helper launch (SteamVR / Meta 32-bit loaders are often absent).
    ...
    if (result.openXrLoaderAvailable)
        ...
        result.message = "OpenXR helper backend selected; 32-bit loader probe skipped (" + loaderDetail + ")";
    return result;
}
