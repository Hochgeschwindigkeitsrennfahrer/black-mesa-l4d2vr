...
public:
	...
	// CEngineClient vtable in engine.dll (image 0x10000000): slot 51 at
	// 0x100A6860 returns "Dedicated Server" or cl.m_szLevelName; slot 52 at
	// 0x100A6890 returns "dedicated" or the short map name (background01).
...
public:
	...
	Vector angles; //0x00C8
	float zNear; //0x00D4
	float zFar; //0x00D8
	float zNearViewmodel; //0x00DC
	float zFarViewmodel; //0x00E0
	char pad_00E4[4]; //0x00E4
	float m_flAspectRatio; //0x00E8
	char pad_00EC[0x5C]; //0x00EC
}; //Size: 0x0148  (copy ctor last field 0x145; GetViewSetup at CViewRender+0x3FC is a second instance)
...
static_assert(offsetof(CViewSetup, m_flAspectRatio) == 0xE8);
