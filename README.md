...
- **Multicore / QoL (compiled, not user-verified):** `GetMatQueueMode` is the real `IMaterialSystem` vfunc 11 so DXVK's queued Present lock can run. `AutoMatQueueMode` switches 0 in menu/load/pause and 2 in gameplay via `SetThreadMode` (never `ClientCmd`). Crash-sticky `mat_queue`. ICvar / `bmvr.cfg` turns off motion blur/grain, `engine_no_focus_sleep 0`, restores vis (not `r_portalsopenall`), **disables BM CSM** (`cl_csm_enabled 0`), drops LOD, and caps squash-blit eyes to the window long-edge. Xen maps (`bm_c4*`) use `r_lod 3`. `DrawModelExecute` scales viewmodels and can hide the local world-model (`HideLocalPlayerModel`).
