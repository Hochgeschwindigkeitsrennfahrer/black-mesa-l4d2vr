...
#include "../third_party/l4d2vr/dxvk_new/include/vulkan/include/vulkan/vulkan.h"
#include "../third_party/openxr/include/openxr/openxr.h"
#include "../third_party/openxr/include/openxr/openxr_platform.h"
#include "../src/openxr_bridge_protocol.h"
...
namespace
{
    ...
    HANDLE OpenParentProcessHandle(DWORD parentPid)
    {
        if (parentPid == 0)
            return nullptr;
        HANDLE process = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION | SYNCHRONIZE, FALSE, parentPid);
        if (process)
            return process;
        return OpenProcess(SYNCHRONIZE, FALSE, parentPid);
    }

    bool ParentProcessStillAlive(HANDLE parentProcess, DWORD* outWait = nullptr, DWORD* outExitCode = nullptr)
    {
        if (!parentProcess)
            return true;
        ...
        const DWORD wait = WaitForSingleObject(parentProcess, 0);
        if (outWait)
            *outWait = wait;
        // WAIT_FAILED used to be treated as "parent dead", which killed the
        // helper during CreateDevice and made the game wait 45s then quit.
        if (wait == WAIT_FAILED)
            return true;
        ...
        uint32_t locatedCount)
    {
        ...
        config.reserved0 = L4D2VR_OPENXR_RUNTIME_VIEW_FOV_LOCATED;
        config.valid =
    ...
    // CreateDevice only needs recommended width/height. Do not put a fake FOV
    // in this config — InitOpenXR latches texture bounds from it, and a
    // placeholder made both eyes identical (broken fusion).
    L4D2VROpenXrRuntimeViewConfigDesc BuildRecommendedRuntimeViewConfig(
        const std::vector<XrViewConfigurationView>& viewConfigs)
    {
        L4D2VROpenXrRuntimeViewConfigDesc config{};
        if (viewConfigs.size() < L4D2VR_OPENXR_EYE_COUNT)
        ...
        config.viewCount = L4D2VR_OPENXR_EYE_COUNT;
        config.reserved0 = L4D2VR_OPENXR_RUNTIME_VIEW_SIZE_ONLY;
        for (uint32_t eye = 0; eye < L4D2VR_OPENXR_EYE_COUNT; ++eye)
        {
            const XrViewConfigurationView& viewConfig = viewConfigs[eye];
            L4D2VROpenXrRuntimeViewDesc& out = config.views[eye];
    ...
    private:
        ...
        bool CreateInstance()
        {
            ...
            std::snprintf(createInfo.applicationInfo.engineName, XR_MAX_ENGINE_NAME_SIZE, "Black Mesa");
        ...
        bool CreateSessionObjects()
        {
            ...
            {
                const L4D2VROpenXrRuntimeViewConfigDesc recommended =
                    BuildRecommendedRuntimeViewConfig(m_ViewConfigs);
                if (recommended.valid)
                {
                    m_Bridge.PublishRuntimeViewConfig(recommended);
                    m_Log.Print(
                        "Published recommended runtime view size %ux%u (FOV not located yet)",
                        recommended.views[0].width,
                        recommended.views[0].height);
                }
            }
        ...
        {
            return ParentProcessStillAlive(m_ParentProcess);
        }
...
float4 main(float4 position : SV_Position, float2 uv : TEXCOORD0) : SV_Target
{
    return sourceTexture.Sample(sourceSampler, uv);
        ...
        {
            return OpenParentProcessHandle(parentPid);
        }
        ...
        const char* stickName)
    {
        ...
        // Black Mesa G2/Touch (bindings_hpmotioncontroller.json), not L4D2VR's
        // left-click recenter / right-click flashlight.
        AddBinding(b, m_BooleanActions[Index(L4D2VROpenXrActionId::CustomAction1)], (leftStick + "/click").c_str());
        AddBinding(b, m_BooleanActions[Index(L4D2VROpenXrActionId::ResetPosition)], (rightStick + "/click").c_str());
    ...
    void OpenXrInputBridge::SuggestBindings(Logger& log)
    {
        ...
        }
        {
            ...
            AddPoseAndHapticBindings(b);
            AddStickAndTriggerBindings(b, "thumbstick");
            // microsoft/motion_controller has squeeze/click, not squeeze/value.
            // Adding squeeze/value fails the whole profile with PATH_UNSUPPORTED.
    ...
    void OpenXrInputBridge::PublishDerivedDpadActions(L4D2VROpenXrInputStateDesc& outState)
    {
        ...
        if (turn.active)
        {
            // G2 SteamVR dpad: north=Jump, south=CrouchToggle. Do not steal
            // this axis for weapon cycle (X/Y do that on Touch).
            SetDerivedDigital(outState, L4D2VROpenXrActionId::Jump, turn.y > kPress);
        }
    ...
    void OpenXrInputBridge::SynthesizeControllerFingerCurls(L4D2VROpenXrInputStateDesc& outState)
    {
        ...
        synthesizeHand(
            ...
            L4D2VROpenXrActionId::Flashlight);
    }
    ...
    void OpenXrInputBridge::PumpHaptics(BridgeWriter& bridge, Logger& log)
    {
        ...
        for (uint32_t hand = 0; hand < L4D2VR_OPENXR_HAND_COUNT; ++hand)
        {
            ...
            static int s_hapticOkLog;
            if (XR_SUCCEEDED(result) && s_hapticOkLog < 6)
            {
                log.Print("xrApplyHapticFeedback ok hand=%u durNs=%lld amp=%.2f freq=%.1f",
                    hand, static_cast<long long>(vibration.duration),
    ...
    private:
        ...
        int FrameLoop(const Options& options)
        {
            ...
            while (!shouldExit)
            {
                ...
                if (!ParentProcessStillAlive(m_ParentProcess, &parentWait, &parentExitCode))
                {
                    m_Log.Print(
                        "Parent process exited wait=0x%08lX exitCode=%lu submitted=%u compositor=%u",
                        static_cast<unsigned long>(parentWait),
                        static_cast<unsigned long>(parentExitCode),
                        submittedFrames,
                        compositorFrames);
                    break;
                }
            ...
            m_Log.Print(
                "Leaving Vulkan frame loop submitted=%u compositor=%u shouldExit=%u sessionState=%d",
                submittedFrames,
                compositorFrames,
                shouldExit ? 1u : 0u,
            ...
            {
                // Keep this a clean exit so CreateDevice does not treat wait-frame
                // pumping as a crash and immediately spawn a second helper.
                m_Bridge.Update(L4D2VROpenXrBridgeStatus::Completed, 0, compositorFrames,
                    "OpenXR Vulkan session ended before game eye submit");
                return 0;
            }
