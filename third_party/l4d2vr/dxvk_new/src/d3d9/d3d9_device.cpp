...
namespace dxvk {
    ...
    HRESULT STDMETHODCALLTYPE D3D9DeviceEx::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters) {
        ...
        // caller's Windowed flag made it Reset every frame (destroy/recreate
        // swapchain + VR eye textures) → 1 FPS and crash. Keep a local
    ...
    void D3D9DeviceEx::NotifyWindowActivated(HWND window, bool activated) {
        ...
        // BM ships d3d9.deviceLossOnFocusLoss=True. That file overrides the
        // app profile. Alt-tab then marks Lost, Source Reset releases VR eye
        // textures, and the next Submit/Present crashes. Keep the device while
        // this DLL is presenting to SteamVR.
