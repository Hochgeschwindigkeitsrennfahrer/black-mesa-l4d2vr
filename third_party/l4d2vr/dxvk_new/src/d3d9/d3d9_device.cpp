...
namespace dxvk {
    ...
    namespace {
        ...
        static bool BmvrFormatsCompatible(D3DFORMAT a, D3DFORMAT b)
        {
            if (a == b)
                return true;
            if (a == D3DFMT_UNKNOWN || b == D3DFMT_UNKNOWN)
                return true;
            if ((a == D3DFMT_A8R8G8B8 && b == D3DFMT_X8R8G8B8)
                || (a == D3DFMT_X8R8G8B8 && b == D3DFMT_A8R8G8B8))
                return true;
            return false;
        }

        // Keep D3D windowed. Do not write Windowed back into the game's
        // D3DPRESENT_PARAMETERS — Source copies that struct and will Reset
        // forever if it sees Windowed=TRUE while video settings say fullscreen.
        static bool BmvrShouldSkipIdenticalWindowedReset(
            ...
            const D3DPRESENT_PARAMETERS* next)
        {
            ...
            if (!cur || !cur->Windowed)
                return false;
            if (cur->BackBufferWidth < 640 || next->BackBufferWidth < 640)
                return false;
            if (cur->BackBufferWidth != next->BackBufferWidth
                || cur->BackBufferHeight != next->BackBufferHeight)
                return false;
            if (cur->EnableAutoDepthStencil != next->EnableAutoDepthStencil)
                return false;
            if (cur->MultiSampleType != next->MultiSampleType)
                return false;
            if (!BmvrFormatsCompatible(cur->BackBufferFormat, next->BackBufferFormat))
                return false;
            return true;
        }
        ...
        static void BmvrKeepDesktopWindowed(D3DPRESENT_PARAMETERS* params)
        {
            ...
            {
                Logger::info(str::format(
                    "BMVR refusing exclusive fullscreen Reset ",
                    params->BackBufferWidth, "x", params->BackBufferHeight,
                    " — keeping Windowed=TRUE (SteamVR + ChangeDisplaySettings loop)"));
                ++s_log;
            }
        ...
        static bool VrForceSwapchainBackBufferToEyeSize(
            ...
            // Black Mesa: HMD-sized swapchain blacks the desktop and leaves
            // SteamVR in the waiting room after Reset. Keep the game's window
            // size unless the hmd_swap retry is still enabled.
            if (!bmvr::TryHmdSwapchain())
                return false;
    ...
    void D3D9DeviceEx::DrawQueuedEyeSubmitOverlays(VR* vr) {
        ...
        const bool manageScene = !m_inScene;
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters) {
    // Source applies video settings through IDirect3DDevice9::Reset.
    // Black Mesa's saved mode is exclusive fullscreen; mutating the
    // caller's Windowed flag made it Reset every frame. Keep a local
    // windowed copy and skip identical Resets.
    ...
    if (BmvrShouldSkipIdenticalWindowedReset(m_implicitSwapchain.ptr(), &local)
     || BmvrShouldSkipWindowedSizeReset(m_implicitSwapchain.ptr(), &local)) {
      ...
      if (s_skipLog < 4) {
        Logger::info(str::format(
          "BMVR skipping identical windowed Reset ",
          local.BackBufferWidth, "x", local.BackBufferHeight));
        ++s_skipLog;
      }
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::SetViewport(const D3DVIEWPORT9* pViewport) {
    ...
    // Only rewrite when RT0 is one of our private eye/HUD surfaces.
    ...
    m_state.viewport.X = effectiveViewport.X;
    m_state.viewport.Y = effectiveViewport.Y;
    m_state.viewport.Width = effectiveViewport.Width;
    m_state.viewport.Height = effectiveViewport.Height;
    m_state.viewport.MinZ = effectiveViewport.MinZ;
    m_state.viewport.MaxZ = effectiveViewport.MinZ < effectiveViewport.MaxZ
      ? effectiveViewport.MaxZ
      : effectiveViewport.MinZ + 0.001f;
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::CheckDeviceState(HWND hDestinationWindow) {
    ...
    HRESULT STDMETHODCALLTYPE D3D9DeviceEx::PresentEx(
        ...
        if (g_Game && g_Game->m_VR && g_Game->m_VR->m_CreatedVRTextures &&
            g_Game->m_VR->m_StereoRenderViewActive) {
            VR* vr = g_Game->m_VR;
  ...
  void D3D9DeviceEx::NotifyWindowActivated(HWND window, bool activated) {
    ...
    if (g_Game && g_Game->m_VR && g_Game->m_VR->m_IsVREnabled)
      return;
